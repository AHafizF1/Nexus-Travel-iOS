...
struct ThemeScreen: View { let viewModel: PreferencesViewModel; var body: some View { Form { Picker("Theme", selection: Binding(get: { viewModel.state.value.theme }, set: saveTheme)) { ForEach(ThemePreference.allCases, id: \.self) { Text(humanizeProfile($0.rawValue)).tag($0) } } }.navigationTitle("Theme") }; private func saveTheme(_ theme: ThemePreference) { var next = viewModel.state.value; next.theme = theme; Task { try? await viewModel.save(next) } } }
