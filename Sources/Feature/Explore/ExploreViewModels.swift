...
@MainActor @Observable final class ExploreViewModel {
    private(set) var state = ExploreUiState(); private let repository: any ExploreRepository
    init(repository: any ExploreRepository) { self.repository = repository }
    func load(forceRefresh: Bool = false) async throws { let prior = state; state.loading = state.content == nil; state.refreshing = state.content != nil; state.error = nil; do { switch try await repository.content(forceRefresh: forceRefresh) { case let .success(value): state = .init(content: value, loading: false); case .empty: state = .init(content: .init(banners: [], destinations: [], packages: []), loading: false); default: state.loading = false; state.refreshing = false; state.error = "Could not update Explore." } } catch is CancellationError { state = prior; throw CancellationError() } }
}
