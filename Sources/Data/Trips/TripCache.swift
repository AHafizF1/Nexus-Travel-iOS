...
actor TripCache {
    struct Entry: Codable, Sendable { let data: Data; let fetchedAt: Date }
    ...
    init(now: Date? = nil, directory: URL? = nil) {
        ...
        fileURL = root.appending(path: "trip-cache.json")
    ...
    func clear() { entries.removeAll(); try? FileManager.default.removeItem(at: fileURL) }
