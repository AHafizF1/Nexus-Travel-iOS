...
Windows cannot run iOS Swift tests. RED and GREEN are separate latest-SHA CI runs on same PR.
...
3. Push, open draft PR, wait for `build-test` failure.
4. Record run URL plus first missing-symbol compiler error in PR body. Failure must come from absent production API, not project setup.
...
3. Push same branch; wait for latest-SHA `build-test` green.
4. Record run URL and test summary in PR body.
...
- [ ] RED CI proves missing production contract.
